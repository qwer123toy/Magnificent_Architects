...
public class BaseMainFrame extends JFrame implements ActionListener {
	...
	private NewsPnl newsPnl;

	public BaseMainFrame(UserInfo userInfo) {
		this.userInfo = userInfo;
		newsInfoNum = (int) (Math.random() * 29) + 1;
...
//					}
					...
					if (upAndDown) {
						JOptionPane.showMessageDialog(BaseMainFrame.this, newsInfo);
						System.out.println("넣기전 " + newsInfo);
						newsInfo = news.getInfo_NewsUp();
						System.out.println("넣은 후 " + newsInfo);
					}

					else {
						JOptionPane.showMessageDialog(BaseMainFrame.this, newsInfo);
						System.out.println("넣기전 " + newsInfo);
						newsInfo = news.getInfo_NewsDown();
						System.out.println("넣은 후 " + newsInfo);
					}
					newsCategory = news.getCompanyCategory();
	...
	private void setPnlCenter() {
		...
		// 매수 패널
		buyPriceGUI = new BuyPriceGUI(userInfo, "A 회사", 0, seeMyTradingHistoryPnl);

		// 매도 패널
		sellPriceGUI = new SellPriceGUI(userInfo, "A 회사", 0);

		newsPnl = new NewsPnl(newsInfoNum);
		News news = listAndDAO.newsDAO.findCompByID(newsInfoNum);
		newsCategory = news.getCompanyCategory();
		boolean upAndDown = true;
		...
		for (int i = 0; i < userMoneyHistoryList.size(); i++) {
			...
			UserMoneyHistory umhForNews = listAndDAO.usermoneyHistoryDAO.findByCompany(
					userMoneyHistoryList.get(i).getUser_Stock(), userInfo.getUser_ID(), userInfo.getUser_SaveData());
			String companyName = umhForNews.getUser_Stock();
			// 뉴스의 회사 이름과 입력받은 부분 비교 필요
			CompanyInfo companyInfo = listAndDAO.companyDAO.findByCompanyName(companyName);
			String companyCategory = companyInfo.getCompanyCategory();
